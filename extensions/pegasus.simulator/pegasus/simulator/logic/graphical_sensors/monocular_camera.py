...
class MonocularCamera(GraphicalSensor):
    ...
    def __init__(self, camera_name, config={}):
        ...
        super().__init__(
            sensor_type="MonocularCamera", update_rate=config.get("frequency", 60.0)
        )
        ...
        self._intrinsics = config.get(
            "intrinsics",
            np.array([[958.8, 0.0, 957.8], [0.0, 956.7, 589.5], [0.0, 0.0, 1.0]]),
        )
        self._distortion_coefficients = config.get(
            "distortion_coefficients",
            np.array([0.14, -0.03, -0.0002, -0.00003, 0.009, 0.5, -0.07, 0.017]),
        )
